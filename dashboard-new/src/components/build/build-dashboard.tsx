...
function SmartCombinationPanel({ build, skill, sprites, baseMods, onApply, onTreeApply }: { build: BuildRow; skill: SkillGroup; sprites: Record<string, string>; baseMods: any; onApply: (items: Partial<Record<SlotKey, ItemDetail>>) => void; onTreeApply: (nodes: string[]) => void }) {
  ...
  const [treeStats, setTreeStats] = useState<{ generated: number; connected: boolean; maxDepth: number; frontierRemaining: number; travel: number; travelRatio: number; regions: number; proposals?: number; beamWidth?: number } | null>(null)
  ...
  return <section className="panel smart-generator">
    ...
    {treeVersion > 0 && <><div className="smart-tree-result"><b>Rota gerada: {treeNodes.length} nós</b><span>Classe: {build.class} · {treeStats?.connected ? 'conectada' : 'inválida'} · travel {treeStats?.travel ?? 0} ({Math.round((treeStats?.travelRatio ?? 0) * 100)}%) · {treeStats?.regions ?? 0} regiões · {treeStats?.proposals ?? 0} propostas</span><button disabled={!treeStats?.connected} onClick={() => onTreeApply(treeNodes)}>Aplicar rota à build</button></div><PassiveTree build={{ ...build, nodes: treeNodes, points_used: treeNodes.length }} skill={skill} /></>}
