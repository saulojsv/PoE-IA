...
function SmartCombinationPanel({ build, sprites, baseMods, onApply, onTreeApply }: { build: BuildRow; sprites: Record<string, string>; baseMods: any; onApply: (items: Partial<Record<SlotKey, ItemDetail>>) => void; onTreeApply: (nodes: string[]) => void }) {
  ...
  return <section className="panel smart-generator">
    ...
    {treeVersion > 0 && <div className="smart-tree-result"><b>Rota gerada: {treeNodes.length} nós</b><span>Classe: {build.class} · seed nova a cada tentativa</span><button onClick={() => onTreeApply(treeNodes)}>Aplicar rota à build</button></div>}
...
export function BuildDashboard() {
  ...
  return <div className="dashboard react-dashboard">
    ...
    <div className="dashboard-main">
      ...
      {stage === 'smart-combination' && (generationCatalog ? <SmartCombinationPanel build={build} sprites={bundle.sprites} baseMods={generationCatalog} onApply={items => { setOverrides(items); setStage('items') }} onTreeApply={nodes => { setTreeOverride(nodes); setStage('tree') }} /> : <section className="panel loading">Carregando catálogo de modificadores apenas para o gerador...</section>)}
