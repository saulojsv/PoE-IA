...
function SmartCombinationPanel({ build, skill, sprites, baseMods, onApply, onTreeApply }: { build: BuildRow; skill: SkillGroup; sprites: Record<string, string>; baseMods: any; onApply: (items: Partial<Record<SlotKey, ItemDetail>>) => void; onTreeApply: (nodes: string[]) => void }) {
  ...
  return <section className="panel smart-generator">
    ...
    {treeVersion > 0 && <><div className="smart-tree-result"><b>Rota gerada: {treeStats?.paidPoints ?? treeNodes.length}/{treePoints} pontos pagos</b><span>Classe solicitada: {treeStats?.requestedClass || 'n/a'} · resolvida: {treeStats?.resolvedClass || 'n/a'} · start {treeStats?.startNodeId || 'n/a'} · fallback {treeStats?.fallbackUsed ? 'sim' : 'não'}</span><span>Seed {treeStats?.seed?.toFixed(5) ?? 'n/a'} · {treeStats?.connected ? '1 componente' : 'inválida'} · travel real {treeStats?.travelByReason ?? treeStats?.travel ?? 0} ({Math.round((treeStats?.travelRatio ?? 0) * 100)}%) · investimento {treeStats?.investment ?? 0} · regiões estratégicas {treeStats?.regions ?? 0}</span><span>Clusters: {treeStats?.completedClusters ?? 0} concluídos · {treeStats?.travelOnlyClusters ?? 0} passagem · {treeStats?.incompleteClusters ?? 0} incompletos · {treeStats?.touchedClusters ?? 0} tocados</span><span>Auditoria: {treeStats?.badLeaves ?? 0} folhas ruins · {treeStats?.redundantNodes ?? 0} redundantes · {treeStats?.proposalsAccepted ?? treeStats?.proposals ?? 0} aceitas · {treeStats?.proposalsRejected ?? 0} rejeitadas</span><button disabled={!treeStats?.connected} onClick={() => onTreeApply(treeNodes)}>Aplicar rota à build</button></div><PassiveTree build={{ ...build, class: treeStats?.resolvedClass || build.class, nodes: treeNodes, points_used: treeNodes.length }} skill={skill} /></>}
