...
import { connectedSelection, disconnectedNodes, generateRandomTree, loadPassiveTree } from '../../data/passive-tree'
...
export function PassiveTreeEditor({ nodes, className, onChange }: { nodes: string[]; className: string; onChange: (nodes: string[]) => void }) {
  ...
  const connected = useMemo(() => connectedSelection(nodes, tree, className), [nodes, tree, className])
  const connectedSet = useMemo(() => new Set(connected), [connected])
  const selected = connectedSet
  ...
  const toggle = (id: string) => onChange(connectedSet.has(id) ? connected.filter(node => node !== id) : [...connected, id])
  ...
  const links = visible.flatMap(node => node.out.filter(id => connectedSet.has(node.id) && connectedSet.has(id) && tree.nodes[id]).map(id => ({ from: node, to: tree.nodes[id] })))
