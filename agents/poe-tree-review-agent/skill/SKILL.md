...
- Causal-tree rule: for every relevant node and route, explain why it is located there and why this build uses it: graph/path role, distance and opportunity cost, main skill/tags, scaling, defenses/resources, attributes, items, configuration, ascendancy and level timing. Separate build-specific causality from reusable patterns and from mere frequency.
- Causal-reading method: infer a node's role from four linked evidence layers—(1) graph path and alternatives, (2) build dependencies such as skills/tags/items/configuration, (3) expected metric affected, and (4) controlled ablation or replacement. Record `node_role`, `depends_on`, `hypothesis`, `test_variant`, `metric_delta`, `verdict`, and `scope`; if ablation is impossible, label the explanation as a hypothesis rather than fact.
